---
title: "Magic Docs Background Maintenance"
owners: []
soft_links: [/platform-services/background-housekeeping-and-deferred-maintenance.md, /runtime-orchestration/query-loop.md, /tools-and-permissions/path-and-filesystem-safety.md, /integrations/plugins/skill-improvement-detection-and-apply-flow.md]
---

# Magic Docs Background Maintenance

Claude Code has an ant-only background documentation-maintenance service for markdown files that opt in with a special header. Once such a file has been read, the runtime can keep it current by periodically launching a tightly constrained internal agent that rewrites the file in place from the latest conversation context. A faithful rebuild needs the tracking contract, idle-only scheduling, and file-edit sandbox to stay intact, or the feature will either never fire, spam on every turn, or escape into arbitrary file mutation.

## Scope boundary

This leaf covers:

- how a markdown file becomes a tracked Magic Doc
...
- generic file-edit tool semantics beyond the Magic Docs-specific restriction boundary

## Opt-in document contract

Equivalent behavior should preserve:

- Magic Docs being opt-in per file rather than a global "keep docs updated" mode
- opt-in detection being based on a markdown header line shaped like `# MAGIC DOC: <title>`
...
- Magic Docs being discovered when the file is read through the ordinary file-read tool path, not by recursively scanning the whole repository for special headers
...
- Magic Docs registering a background post-sampling hook rather than editing docs inline during the foreground response
- the service only being active for ant users
...
- docs that no longer contain the Magic Doc header also being removed from tracking
...
- Magic Docs updates running through a built-in agent path rather than by directly string-replacing markdown from the hook
...
- only `Edit` operations against the tracked Magic Doc path being allowed
...
- preserving the Magic Doc header exactly
...
- a user-level custom prompt override file at `~/.claude/magic-docs/prompt.md`
- silent fallback to the built-in default prompt when that override file is missing or unreadable
- variable substitution using placeholders such as `{{docContents}}`, `{{docPath}}`, `{{docTitle}}`, and `{{customInstructions}}`
...
- Magic Docs only tracking documents the user actually touched in the session
...
- Magic Docs remaining advisory automation over documentation, not a replacement for ordinary source of truth in the codebase itself

## Failure modes

- **never-tracked docs**: the implementation scans nothing on file reads or forgets to register matching files, so Magic Docs silently never update
- **stale rereads**: the updater trusts cached `file_unchanged` state and misses the current file contents before issuing edits
- **header drift**: the updater rewrites or deletes the Magic Doc header or its inline instruction line and thereby destroys the document's future opt-in metadata
