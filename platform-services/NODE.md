...
- **[session-cost-accounting-and-restoration.md](session-cost-accounting-and-restoration.md)** — How provider-resolved usage becomes one session spend ledger, how unknown pricing and fast-mode tiers are handled, and how that ledger is saved, restored, or cleared across resume, session switches, and auth changes.
...
- **[opt-in-doc-maintenance-background-agent.md](opt-in-doc-maintenance-background-agent.md)** — How opt-in markdown docs are discovered, tracked, and rewritten by a constrained background agent that keeps docs current instead of historical.
- **[mock-rate-limit-scenarios-and-test-contracts.md](mock-rate-limit-scenarios-and-test-contracts.md)** — Testable contracts for the 20 mock rate limit scenarios used in testing, with full header specifications and acceptance criteria.
