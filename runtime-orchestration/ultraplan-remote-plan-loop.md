---
title: "Ultraplan Remote Plan Loop"
owners: []
soft_links: [/ui-and-experience/plan-mode-approval-surfaces.md, /runtime-orchestration/remote-agent-restoration-and-polling.md, /runtime-orchestration/task-model.md, /product-surface/interaction-modes.md, /integrations/clients/direct-connect-session-bootstrap-and-environment-selection.md]
---

# Ultraplan Remote Plan Loop

Ultraplan is not just a long-running remote task. It is a planning-specific remote loop that begins from a local prompt or approval surface, launches a remote plan-mode session, polls for approval artifacts, and then splits into either remote execution or local handoff.

## Trigger and prelaunch contract

Equivalent behavior should preserve:

- bare `/ultraplan` acting as a usage query rather than a launch attempt
- non-empty `/ultraplan` invocations and keyword-triggered prompts funneling into the same launch path
...
The important contract is that Ultraplan begins as a live control surface, not just a slash-command text expansion.

## Singleton launch guard and immediate feedback

Equivalent behavior should preserve:

- a singleton guard that blocks launch when an Ultraplan session is already active or already launching
...
- scaffolding deliberately avoiding the feature name itself so the remote CLI's keyword detector does not self-trigger
...
This is why Ultraplan cannot be reconstructed as "remote plan command" alone. Its bootstrap payload is feature-specific.
...
- registering the remote session as a local shadow task typed specifically as Ultraplan rather than as an ordinary remote agent
...
- three user-visible Ultraplan phases derived from remote events plus session status: `running`, `plan_ready`, and `needs_input`
...
- **stale cleanup**: an older failed poll clears the URL or state for a newer relaunched Ultraplan session
