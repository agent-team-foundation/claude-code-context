---
title: "Feature-Gated Skill Search and Canonical Skill Flow"
owners: []
...
# Feature-Gated Skill Search and Canonical Skill Flow

The observed snapshot contains a second, feature-gated skill-discovery layer beyond the ordinary local skill registry. It does not replace bundled, local, plugin, or MCP skills. Instead, it supplements them with per-turn discovery signals and a model-only path for previously discovered remote canonical skills. A faithful rebuild therefore needs a separate contract for discovery timing, surfaced reminders, session state, and remote-skill execution, even though the backend retrieval modules are absent from this snapshot.
...
- the model-only execution path for previously discovered remote canonical skills
...
- the hidden retrieval, ranking, and remote-storage backend for discovery, which is a clean-room gap in this snapshot

## Skill search is an overlay for gaps, not a replacement for visible skills
...
- the discovery path treating already visible or already loaded skills as duplicates to filter away rather than re-surfacing them pointlessly
- static eager skill listings narrowing to bundled and MCP skills when this mode is active, so long-tail project, user, and plugin skills can arrive through discovery instead of bloating the base prompt

## Main sessions and subagents both receive explicit discovery framing
...
- workers therefore understanding discovery reminders even when they only encounter them after spawn-time prompt enhancement

## Discovery has two signal paths with different timing

Equivalent behavior should preserve:

- user input on turn zero being able to trigger a blocking discovery pass before the first model request, because there is no ongoing work to hide that latency under
- later turns starting discovery asynchronously while the model streams and tools run
- that inter-turn discovery prefetch being tied to the current turn trajectory, especially write-oriented work, rather than acting as a blind every-turn catalog refresh
...
- plan-exit notifications not re-triggering discovery, because the relevant planning request should already have provided the right signal earlier in the turn
...
- rebuilds treating this as a correctness requirement rather than a mere optimization, because otherwise every large `SKILL.md` expansion can trigger redundant discovery work and surface irrelevant reminders

## Discovery results are surfaced as system-reminder attachments, not registry mutations
...
## Discovered-skill state is scoped differently in interactive and headless runtimes

Equivalent behavior should preserve:

- interactive REPL sessions keeping a discovered-skill set across multiple turns so later skill invocations can still count as previously discovered
- clearing a conversation also clearing that discovered-skill set, alongside other session-scoped context caches
- QueryEngine-style headless or SDK submits clearing their discovered-skill set at the start of each submitted message, so discovery state does not grow forever across unrelated turns
...
## Previously discovered remote canonical skills are model-only and session-gated

Equivalent behavior should preserve:

- remote canonical skills not living in the local command registry
...
- remote canonical skills remaining model-facing rather than becoming normal user slash commands

## Remote canonical execution bypasses local slash compilation

Equivalent behavior should preserve:

- the remote canonical path short-circuiting before ordinary local command lookup
...
This snapshot exposes the prompt guidance, attachment timing, discovered-name state, and remote-execution behavior around feature-gated skill search, but it does not include the underlying discovery backend modules themselves. Rebuilds should therefore preserve:
...
- **guessed remote execution**: a remote canonical skill can be invoked without prior discovery, so the model can call hidden names by guesswork
- **local-remote conflation**: remote canonical skills are run through ordinary local prompt compilation and accidentally gain shell expansion or argument interpolation
